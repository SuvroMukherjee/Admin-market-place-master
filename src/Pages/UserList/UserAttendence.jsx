          ...
          <div>
            <span className="loglocation">
              {params?.row?.log_in_time
                ? moment
                    .tz(params?.row?.log_in_time, "Asia/Kolkata")
                    .format("DD-MM-YYYY")
                : ""}
            </span>
          </div>
...
, "Asia/Kolkata").format('HH:mm:ss A') : 'Invalid Time'}</span> */}
        ...
        return (
          <div className="productListItem">
            {params?.row?.log_out_time && (
              <span className="logoutT">
                {moment
                  .tz(params?.row?.log_out_time, "Asia/Kolkata")
                  .format("dddd")}
              </span>
            )}
          </div>
  ...
  const { log_in_time, log_in_loc, log_out_time, log_out_loc } =
    attendenceadata || {};

  // Calculate date and day for login and logout times if data exists
  const loginDate = log_in_time ? formatDate(log_in_time) : "";
  const loginDay = log_in_time
    ? new Date(log_in_time).toLocaleDateString("en-US", { weekday: "long" })
    : "";
  const logoutDate = log_out_time ? formatDate(log_out_time) : "";
  const logoutDay = log_out_time
    ? new Date(log_out_time).toLocaleDateString("en-US", { weekday: "long" })
    : "";

  // Calculate time difference if login and logout times exist
  const timeDiff =
    log_in_time && log_out_time
      ? getTimeDifference(log_in_time, log_out_time)
      : "";
  ...
  if (loginDate && loginDay && logoutDate && logoutDay && timeDiff) {
    tableData = [
      [
        loginDate,
        loginDay,
        log_in_time.slice(11, 16),
        log_in_loc?.location,
        log_out_time.slice(11, 16),
        log_out_loc?.location,
        timeDiff,
      ],
    ];
  }

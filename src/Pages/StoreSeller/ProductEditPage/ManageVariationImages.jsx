...
const ManageVariationImages = ({
  ...
  async function getProductdata() {
    ...
    if (res?.data?.data?.additionalSpec?.length > 0) {
      let oldImageList = res?.data?.data?.SellerProductData?.specId?.image;
      let newImageList = res?.data?.data?.additionalSpec
        ?.map((item) => {
          return item?.image;
        })
        .flat();
      setAllImageList([...oldImageList, ...newImageList]);
    } else {
      let oldImageList = res?.data?.data?.SellerProductData?.specId?.image;
      setAllImageList(oldImageList);
    }
  }
  ...
  const onFileUpload = async (file) => {
    ...
    try {
      ...
      sellerProductDeatils(productId);
    } catch (err) {
    ...
    <Container>
      <Row className="mx-4">
        <Col className="text-center infotab">
          ...
          When multiple sellers sell the same product through a single detail
          page, we combine and present the best product data to ensure customers
      ...
      <Row className="m-4 p-4 justify-content-md-start stepContent paddingConatiner">
        <Row className="mt-2">
          ...
          <Col className="live2 mt-2">
            {" "}
            This images are currently used by Zoofi as part of this product
            listing.
          </Col>
        </Row>
        ...
        <Row className="mt-4">
          <Col xs={12} className="live3 mt-2">
            Please upload your Recommanded Images for your prosduct
          </Col>
          ...
          <Row className="mt-4 mb-4">
            <Col className="d-flex justify-content-end">
              <Button type="submit" onClick={() => handleSubmit()}>
                Add <span className="imgeCount">{formData?.image?.length}</span>{" "}
                Images
              </Button>
            </Col>
          </Row>
